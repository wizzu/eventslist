...
## Responsive Design
- [ ] Single-column mobile layout — primary goal is mobile where the two columns stack vertically; the user would then have to scroll past the entire event list to reach the stats panel. The side-by-side layout works well when the display/window is wide enough (desktop or wide browser window), so responsive work only kicks in below some breakpoint. Narrow desktop browser windows are also a valid scenario but don't fundamentally change the approach.
  - Collapsible event listing: collapse control (chevron) added directly after the "N events" / "N event" text in the toolbar — the count and chevron together are the clickable affordance. Style the event count more like a section heading (larger, darker, less muted) so it reads as a header. Only `div.event-list` and the scroll-to-top button collapse; search box and toolbar always stay visible so the user can still search while the list is hidden. On desktop the collapse is less useful (both panels visible side by side) — implement it as part of responsive work, not as a standalone desktop feature. The Alpine toggle state and HTML collapse mechanism are straightforward and reusable across breakpoints; the main effort is the CSS layout changes.
  - Column width on desktop: when the listing collapses, the flex ratio (65/35) should keep widths stable since they're grow-ratio-based not content-based — verify visually. On mobile (single column) this question doesn't apply.
  - Listing vs stats view: evaluate whether single-column layout needs an explicit toggle between the listing and stats views, or whether stacking with collapsible sections (stats sections already collapsible, event listing to be collapsible) is sufficient.
